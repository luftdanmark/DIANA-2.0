...
class NaiveBayes(Classifier):
    ...
    def __init__(self, dataObj=None, headers=[], categories=None):
        ...
        self.num_classes = 0
        self.num_features = len(self.headers)
        ...
        if dataObj != None:
            self.build(dataObj.getData(), categories)
    ...
    def build( self, A, categories ):
        ...
        unique = np.unique( np.array(categories.T), return_inverse=True)
        self.num_classes = unique.size[0]
